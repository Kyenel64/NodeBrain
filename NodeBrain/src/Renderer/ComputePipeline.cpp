...
namespace NodeBrain
{
	std::shared_ptr<ComputePipeline> ComputePipeline::Create(std::shared_ptr<Shader> computeShader)
	{
		switch (Renderer::GetGAPI())
		{
			case GAPI::None: NB_ERROR("Graphics API not detected"); return nullptr;
			case GAPI::Vulkan: return std::make_shared<VulkanComputePipeline>(std::dynamic_pointer_cast<VulkanShader>(computeShader));
		}
