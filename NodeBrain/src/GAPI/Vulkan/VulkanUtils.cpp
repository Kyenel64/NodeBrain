...
namespace NodeBrain::Utils
{
	...
	void CopyImageToImage(VkCommandBuffer commandBuffer, VkImage srcImage, VkImage dstImage, VkExtent2D srcExtent, VkExtent2D dstExtent)
	{
		...
		blitRegion.srcOffsets[1].x = (int)srcExtent.width;
		blitRegion.srcOffsets[1].y = (int)srcExtent.height;
		blitRegion.srcOffsets[1].z = 1;

		blitRegion.dstOffsets[1].x = (int)dstExtent.width;
		blitRegion.dstOffsets[1].y = (int)dstExtent.height;
